...
uint32_t HashTableDirectoryPage::GetSplitImageIndex(uint32_t bucket_idx) {
  throw NotImplementedException(
      "Split image index will be confusing when splitting or merging, not gonna be implemented.");
}
...
bool HashTableDirectoryPage::CanShrink() {
  uint32_t size = Size();
  for (uint32_t idx = 0; idx < size; idx++) {
    if (global_depth_ <= local_depths_[idx]) {
      return false;
    }
  }
  return true;
...
uint32_t HashTableDirectoryPage::GetPairIndex(uint32_t bucket_idx) {
