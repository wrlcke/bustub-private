 ...
 public:
  ...
  T GetElement(int i, int j) const override {
    if (i >= this->rows_ || i < 0 || j >= this->cols_ || j < 0) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "access element out of range");
    }
    return data_[i][j];
  }
  ...
  void FillFrom(const std::vector<T> &source) override {
    ...
    if (source.size() != static_cast<size_t>(size)) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "the size of the source does not match the size of the matrix");
    }
 ...
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    if (matrixA->GetRowCount() == matrixB->GetRowCount() && matrixA->GetColumnCount() == matrixB->GetColumnCount()) {
      ...
      auto sum_ptr = std::make_unique<RowMatrix<T>>(row, col);
      for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
          sum_ptr->SetElement(i, j, matrixA->GetElement(i, j) + matrixB->GetElement(i, j));
        }
      }
      return sum_ptr;
    }
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    if (col_a == row_b) {
      auto product_ptr = std::make_unique<RowMatrix<T>>(row_a, col_b);
      for (int i = 0; i < row_a; i++) {
        for (int j = 0; j < col_b; j++) {
          ...
          product_ptr->SetElement(i, j, sum);
        }
      }
      return product_ptr;
    }
